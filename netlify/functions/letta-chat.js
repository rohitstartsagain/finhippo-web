...
export default async (req, context) => {
  try {
    ...
    const body = {
      messages: [
        {
          type: "message",          // <-- required discriminator
          role: "user",
          content: [
            { type: "text", text: message } // <-- tagged content item
          ]
          // sender_id: "identity-xxxx", // optional when you add identities later
        }
      ]
    };
    ...
    // 5) Extract assistant reply (collect text parts from returned messages)
    const reply =
